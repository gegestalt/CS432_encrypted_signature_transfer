...
namespace Server_project_432
{
    public partial class Form2 : Form
    {
        ...
        {
            string file_path = @"C:\Users\guney\Downloads\revised_432_test\Intro432Proje\Server2_project_432\bin\Debug\net5.0-windows\files";
        ...
        {
            string filePath = @"C:\Users\guney\Downloads\revised_432_test\Intro432Proje\Server2_project_432\bin\Debug\net5.0-windows\files";
        ...
        private void server1_connect_button_Click(object sender, EventArgs e)
        {
            ...
            //IP = "127.0.0.1";
            //port = 111;
            if (Int32.TryParse(server1_port_textBox.Text, out port)) //Int32.TryParse(rmt_port.Text, out port)
            {
                try
                {
        ...
        private void Receive()
        {
            ...
            while (connected && !terminating)
            {
                try
                {
                    ...
                    if (master_connected && server1_connected && server1_master_connected)
                    {
                        try
                        {
                ...
                catch (Exception e)
                {
                    ...
                    if (connectionTypeList[socketList.Count - 1] == "master_server")
                    {
                        ...
                        //    byte[] master_buffer = Encoding.Default.GetBytes(master_message);
                        //    serverSocket.Send(master_buffer);
                        //}
                    }

                    else if (connectionTypeList[socketList.Count - 1] == "server_1")
                    {
                        logs_server.AppendText("Server-1 is disconnected. \n");
                        server1_connected = false;
                        connectionTypeList.Remove("server_1");

                        if (connectionTypeList.Contains("master_server"))
                        {
                            string master_message = "server_1_2_disconnected";
                            byte[] master_buffer = Encoding.Default.GetBytes(master_message);
                            remoteSocket.Send(master_buffer);
                ...
                if (master_connected && server1_connected && server1_master_connected)
                {
                    try
                    {
                        ...
                        while (not_replicated_files.Count > 0 && not_replicated_file_names.Count > 0)
                        {
                            ...
                            bool isReplicated_master_server = distributeFile(server_1_socket, file_data, file_name, "server_2", "master_server");
                            bool isReplicated_server1 = distributeFile(remoteSocket, file_data, file_name, "server_2", "server_1");
